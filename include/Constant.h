...
// Period of I/Q carrier modulation, defined as samples per carrier cycle
// (Value: 4)
constexpr int DEMOD_SAMPLE_PERIOD = 4;

// --- Derived Sonar & DSP Parameters ---
// ADC sampling rate in Hz, derived from center frequency and demodulation
// period (Value: 160000.0)
...
// DC offset of the ADC, derived as the midpoint of the ADC range (Value:
// 2048.0f)
...
// Total number of ADC samples collected per frame, derived from chunk settings
// (Value: 2048)
constexpr size_t ADC_SAMPLES = CHUNK_SAMPLES * CHUNKS_PER_FRAME;

// Shift factor to compute division by ADC_SAMPLES using bit-shifting (Value:
// 11)
constexpr int ADC_SAMPLES_SHIFT = 11;

// Length of each DMA buffer, matching the size of a single sample chunk (Value:
// 512)
...
// Length of the matched filter coefficients, matching single pulse length
// (Value: 8)
...
// Number of pulses in the slow-time dimension for Doppler processing (Value:
// 16)
...
// Calibration factor to adjust actual physical sampling rate discrepancy
// (Value: 1.10f)
...
// Pre-calculated value of cos(pi/4) scaled in Q15 format, approximately 0.7071
// (Value: 23170)
constexpr int32_t COS_PI_4_Q15 =
    static_cast<int32_t>(0.7071067811865476 * (1 << Q15_SHIFT));
...
// Right shift amount applied after matched filter multiplication/accumulation
// (Value: 13)
constexpr int MATCHED_FILTER_SHIFT = 13;

// Rounding offset added before matched filter scaling shift, equal to
// 2^(shift-1) (Value: 4096)
constexpr int32_t MATCHED_FILTER_ROUND_OFFSET = 1 << (MATCHED_FILTER_SHIFT - 1);

// Squared scale factor for target threshold, representing 1024 squared (Value:
// 1048576)
constexpr int64_t TARGET_THRESHOLD_SCALE_SQ = 1024LL * 1024LL;

// --- Detection Thresholds & Blanking ---
// Base target detection threshold for Barker 13 pulse compressed signals
// (Value: 120)
...
// Number of initial samples to blank to filter out transmitter leakage (Value:
// 220)
constexpr int TX_LEAKAGE_BLANK_SAMPLES = 220;

// Scaling factor applied to signal stream before transmission or display
// (Value: 3)
constexpr int COMPRESSED_STREAM_SCALE = 3;

// --- Waveform Definitions ---
// Pre-calculated single pulse waveform (1 chip = 2 cycles of 4 samples/cycle =
// 8 samples)
constexpr uint8_t SINGLE_PULSE_WAVE[FILTER_COEFFS_LEN] = {127, 254, 127, 0,
                                                          127, 254, 127, 0};
...
// Pulse Repetition Interval (PRI) in milliseconds for single pulse mode (Value:
// 15)
constexpr uint32_t PRI_SINGLE_MS = 15;

// Pulse Repetition Interval (PRI) in milliseconds for Barker 13 mode (Value:
// 20)
...
// Microsecond threshold above which a task yields execution to other tasks
// (Value: 2000)
...
// Maximum iteration count for WiFi connection retry, representing 10s timeout
// (Value: 20)
constexpr uint32_t WIFI_CONNECT_TIMEOUT_LIMIT = 10000 / WIFI_CONNECT_DELAY_MS;

// Delay in milliseconds for ARP resolution before sending UDP packets (Value:
// 300)
...
// 25809)
constexpr int16_t THRESHOLD_SYNC = static_cast<int16_t>(
    (SYNC_THRESHOLD_VOLTS / (ADC_REF_VOLTS / 2.0f)) * Q15_MAX);
...
// Delay in microseconds after stopping I2S/ADC to allow hardware transient
// settling (Value: 50)
constexpr uint32_t ADC_STOP_STABILIZATION_US = 50;

// Delay in milliseconds to hold CPU when UDP transmission fails due to LwIP
...
// Cutoff coefficients for 1-pole sample-to-sample IIR smoothing filter (Value:
// 0.90f / 0.10f)
...
// Delay in milliseconds during setup to allow hardware stabilization (Value:
// 1000)
constexpr uint32_t SETUP_DELAY_MS = 1000;

// FreeRTOS task stack size allocated for application tasks in bytes (Value:
// 8192)
...
// Logarithmic multiplier to scale voltage ratios to decibel levels
// (Value: 20.0f)
