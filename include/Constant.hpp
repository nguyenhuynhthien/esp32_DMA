...
constexpr float CPU_CYCLES_PER_SAMPLE_FACTOR =
    1000000.0f / static_cast<float>(SAMPLE_RATE);
...
// Default peak index to return when peak detection fails (Value: DAC_PULSE_LEN,
// which is 12)
...
constexpr uint32_t WIFI_CONNECT_TIMEOUT_LIMIT =
    WIFI_CONNECT_TIMEOUT_MS / WIFI_CONNECT_DELAY_MS;
